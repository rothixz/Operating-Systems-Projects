 ...
 *      \li convert the index number, which translates to an entry of the references to free data clusters table, into
 *          the logical number (the ordinal, starting at zero, of the succession blocks that the table of references to
 *          free data clusters comprises) and the offset of the block where it is stored
 *      \li load the contents of a specific block of the table of references to free data clusters into internal storage
 *      \li get a pointer to the contents of a specific block of the table of references to free data clusters
 *      \li store the contents of the block of the table of references to free data clusters resident in internal storage
 ...
 *
 *  \author António Rui Borges - August 2010 - August 2012
 */
...
/** \brief storage area for one block of the table of free data clusters */
static uint32_t ref[RPB];
/** \brief validation area: -2 - an error occurred while reading or writing a data block
 *                          -1 - no block of the table of free data clusters has been read yet
 *                           * - logical block number of table of free data clusters that has been read
 */
static int nBlkFCTLoaded = -1;
/** \brief status of reading or writing a data block of the table of free data clusters */
static int fctError = 0;
...
/**
 *  \brief Convert the index number, which translates to an entry of the references to free data clusters table, into the
 *         logical number (the ordinal, starting at zero, of the succession blocks that the table of references to free
 *         data clusters comprises) and the offset of the block where it is stored.
 *
 *  \param ind index to the table of references to free data clusters
 *  \param p_nBlk pointer to the location where the logical block number is to be stored
 *  \param p_offset pointer to the location where the offset is to be stored
 *
 *  \return <tt>0 (zero)</tt>, on success
 *  \return -\c EINVAL, if index is out of range or any of the pointers is \c NULL
 *  \return -\c EBADF, if the device is not already opened
 *  \return -\c EIO, if it fails on reading or writing
 *  \return -\c ELIBBAD, if the buffercache is inconsistent or the superblock was not previously loaded on a previous
 *                       store operation
...
int soConvertRefFCT (uint32_t ind, uint32_t *p_nBlk, uint32_t *p_offset)
{
  soColorProbe (718, "07;31", "soConvertRefFCT (%"PRIu32", %p, %p)\n", ind, p_nBlk, p_offset);

  int stat;                                      /* status of operation */

  if ((stat = soLoadSuperBlock ()) != 0) return stat;
  if ((ind >= sb.dzone_total) || (p_nBlk == NULL) || (p_offset == NULL))
     return -EINVAL;
...
/**
 *  \brief Load the contents of a specific block of the table of references to free data clusters into internal storage.
 *
 *  Any type of previous / current error on loading / storing the data block will disable the operation.
 *
 *  \param nBlk logical number of the block to be read
...
int soLoadBlockFCT (uint32_t nBlk)
{
  soColorProbe (719, "07;31", "soLoadBlockFCT (%"PRIu32")\n", nBlk);

  int stat;                                      /* status of operation */

  if ((stat = soLoadSuperBlock ()) != 0) return stat;
  if (nBlk >= sb.dzone_total) return -EINVAL;

  if (fctError != 0) return fctError;            /* a previous error has occurred */
  if (nBlk == nBlkFCTLoaded) return 0;           /* the block has already been read */
  stat = soReadCacheBlock (sb.tbfreeclust_start + nBlk, ref);
  if (stat == 0)
     nBlkFCTLoaded = nBlk;                       /* operation carried out with success */
     else { nBlkFCTLoaded = -1;
            fctError = stat;                     /* an error has occurred while reading */
          }
...
/**
 *  \brief Get a pointer to the contents of a specific block of the table of references to free data clusters.
 *
 *  Any type of previous / current error on loading / storing the data block will disable the operation.
 *
 *  \return pointer to the specific block , on success
...
uint32_t *soGetBlockFCT (void)
{
  soColorProbe (720, "07;31", "soGetBlockFCT ()\n");
...
/**
 *  \brief Store the contents of the block of the table of references to free data clusters resident in internal storage
 *         to the storage device.
 *
 *  Any type of previous / current error on loading / storing the data block will disable the operation.
 *
...
int soStoreBlockFCT (void)
{
  soColorProbe (721, "07;31", "soStoreBlockFCT ()\n");
  ...
  if (nBlkFCTLoaded < 0)
     { nBlkFCTLoaded = -2;
       fctError = -ELIBBAD;                      /* no block of the bitmap table of inodes has not been
                                                    read yet */
       return fctError;
     }
  stat = soWriteCacheBlock (sb.tbfreeclust_start + nBlkFCTLoaded, ref);
  if (stat != 0)
     { nBlkFCTLoaded = -2;
       fctError = stat;                          /* an error has occurred while writing */
     }
...
{
  soColorProbe (722, "07;31", "soConvertBPIDC (%"PRIu32", %p, %p)\n", p, p_clustInd, p_offset);
...
{
  soColorProbe (723, "07;31", "soLoadSngIndRefClust (%"PRIu32")\n", nClust);
...
{
  soColorProbe (724, "07;31", "soGetSngIndRefClust ()\n");
...
{
  soColorProbe (725, "07;31", "soStoreSngIndRefClust ()\n");
...
{
  soColorProbe (726, "07;31", "soLoadDirRefClust (%"PRIu32")\n", nClust);
...
{
  soColorProbe (727, "07;31", "soGetDirRefClust ()\n");
...
{
  soColorProbe (728, "07;31", "soStoreDirRefClust ()\n");
