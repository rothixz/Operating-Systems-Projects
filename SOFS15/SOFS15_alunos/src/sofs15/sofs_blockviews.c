...
void printSuperBlock (void *buf)
{
  ...
  if (p_sb->itable_start == NULL_BLOCK)
     printf ("(nil)\n");
     else printf ("%"PRIu32"\n", p_sb->itable_start);
  ...
  printf ("   Index of the first / last free inode in the double-linked list (point of retrieval / insertion)  = ");
  ...
  if (p_sb->dzone_start == NULL_BLOCK)
     printf ("(nil)\n");
     else printf ("%"PRIu32"\n", p_sb->dzone_start);
  ...
  if (p_sb->dzone_retriev.cache_idx == DZONE_CACHE_SIZE)
     printf (" empty");
     else for (i = p_sb->dzone_retriev.cache_idx; i < DZONE_CACHE_SIZE; i++)
          { if (p_sb->dzone_retriev.cache[i] == NULL_CLUSTER)
               printf (" (nil)");
               else printf (" %"PRIu32"", p_sb->dzone_retriev.cache[i]);
            if ((((i - p_sb->dzone_retriev.cache_idx + 1) % 10) == 0) && (i < (DZONE_CACHE_SIZE - 1)))
               { printf ("\n");
  ...
  printf ("      Reference cache contents:");
  if (p_sb->dzone_insert.cache_idx == 0)
     printf (" empty");
     else for (i = 0; i < p_sb->dzone_insert.cache_idx; i++)
          { if (p_sb->dzone_insert.cache[i] == NULL_BLOCK)
               printf (" (nil)");
               else printf (" %"PRIu32"", p_sb->dzone_insert.cache[i]);
            if ((((i + 1) % 10) == 0) && (i < (p_sb->dzone_insert.cache_idx - 1)))
               { printf ("\n");
                 printf ("                               ");
               }
          }
  printf("\n");
  printf ("   Physical number of the block where the table of references to data clusters starts  = ");
  if (p_sb->tbfreeclust_start == NULL_BLOCK)
     printf ("(nil)\n");
     else printf ("%"PRIu32"\n", p_sb->tbfreeclust_start);
  printf ("   Number of blocks that the table of references to data clusters comprises  = %"PRIu32"\n",
		  p_sb->tbfreeclust_size);
  printf ("   Reference to the first data cluster in the static circular FIFO of free data\n");
  printf ("     clusters (point of retrieval)  = ");
  if (p_sb->tbfreeclust_head == NULL_CLUSTER)
     printf ("(nil)\n");
     else printf ("%"PRIu32"\n", p_sb->tbfreeclust_head);
  printf ("   Reference to the last data cluster in the static circular FIFO of free data\n");
  printf ("     clusters (point of insertion)  = ");
  if (p_sb->tbfreeclust_tail == NULL_CLUSTER)
     printf ("(nil)\n");
  ...
  if (p_inode->mode & INODE_FREE)
     ...
     { printf ("prev = ");
       if (p_inode->vD1.prev == NULL_INODE)
           printf ("(nil), ");
           else printf ("%"PRIu32", ", p_inode->vD1.prev);
       printf ("next = ");
 ...
 *
 *  The content is displayed as a byte stream in rows of 16 characters each.
 *  Each row is labeled by the address of the first byte, displayed in hexadecimal.
 *  Both decimal and ascii representations are used as required.
 *
...
void printCltDirEnt (void *buf)
{
  ...
  { for (j = 0; j < MAX_NAME+1; j++)
      if ((clust->de[i].name[j] == '\0') || (clust->de[i].name[j] < ' ') ||
          (clust->de[i].name[j] > 0177))
         printf (" ");
         else printf ("%c", clust->de[i].name[j]);
    printf(": ");
    if (clust->de[i].nInode == NULL_INODE)
       printf ("(nil)\n");
...
void printCltRef (void *buf, bool isCluster)
{
  SODataClust *clust;                            /* pointer to a cluster reference */
  int i;                                         /* counting variable */
  ...
  for (i = 0; i < (isCluster ? RPC : RPB); i++)
  { /* print address label, if required */
    if ((i & 0x07) == 0) printf ("%4.4d:", i);
    /* print reference to a cluster */
    if (clust->ref[i] == NULL_CLUSTER)
       printf ("   (nil)   ");
       else printf (" %.10"PRIu32"", clust->ref[i]);
    /* terminate present line, if required */
